...
class ScriptedGateway(PaymentGateway):
    ...
    def __init__(self, results: list[PaymentResult]) -> None:
        # TODO Day 3
        # Store results as a queue (FIFO)
        self._results = list(results)

    def charge(self, invoice: Invoice) -> PaymentResult:
        # TODO Day 3
      if not self._results:
            # If exhausted, raise to signal test misconfiguration
            raise RuntimeError("No more scripted results available")
    ...
    def charge(self, invoice: Invoice) -> PaymentResult:
        # TODO Day 3
         if self._rng.random() < self.success_rate:
            ...
            return PaymentResult(success=False, failure_reason="PAYMENT_FAILED")
